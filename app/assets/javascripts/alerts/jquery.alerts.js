...
(function(jQuery) {
	...
	jQuery.alerts = {
		...
		text: function(message, value, title, callback) {
			if( title == null ) title = 'Text';
			jQuery.alerts._show(title, message, value, 'text', function(result) {
				if( callback ) callback(result);
			});
		},
		noButton: function(message, title, callback) {
			if( title == null ) title = 'Confirm';
			jQuery.alerts._show(title, message, null, 'noButton', function(result) {
				if( callback ) callback(result);
			});
		},
		...
		_show: function(title, msg, value, type, callback) {
			...
			switch( type ) {
				case 'alert':
					jQuery("#popup_message").after('<div id="popup_panel"><input class="medium gray awesome" type="button" value="' + jQuery.alerts.okButton + '" id="popup_ok" /></div>');
				...
				break;
				case 'noButton':
					jQuery("#popup_message").after('<div id="popup_panel"><input class="medium gray awesome" type="button" value="' + jQuery.alerts.okButton + '" id="popup_ok" /> <input class="medium gray awesome" type="button" value="' + jQuery.alerts.cancelButton + '" id="popup_cancel" /></div>');
					jQuery("#popup_ok").click( function() {
						callback(true);
					});
				...
				break;
				case 'prompt':
					jQuery("#popup_message").append('<br /><textarea rows=4 id="popup_prompt" ></textarea>').after('<div id="popup_panel"><input class="medium gray awesome" type="button" value="' + jQuery.alerts.okButton + '" id="popup_ok" /> <input class="medium gray awesome" type="button" value="' + jQuery.alerts.cancelButton + '" id="popup_cancel" /></div>');
				...
				break;
				case 'text':
					jQuery("#popup_message").append("<br /><input type='text' id='popup_prompt' />").after('<div id="popup_panel"><input class="medium gray awesome" type="button" value="' + jQuery.alerts.okButton + '" id="popup_ok" /> <input class="medium gray awesome" type="button" value="' + jQuery.alerts.cancelButton + '" id="popup_cancel" /></div>');
					jQuery("#popup_prompt").width( jQuery("#popup_message").width() );
					jQuery("#popup_ok").click( function() {
						var val = jQuery("#popup_prompt").val();
						jQuery.alerts._hide();
	...
	jText = function(message, value, title, callback) {
		jQuery.alerts.text(message, value, title, callback);
	};
	jNobutton = function(message, title, callback) {
		jQuery.alerts.noButton(message, title, callback);
	};
	
	jNobutton_nego = function(message, title, callback) {
		jQuery.alerts.noButton(message, title, callback);
		gwcNegotiation_exe.update();
	};
